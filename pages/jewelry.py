...
bracelet_images = ["Jewelry/assets/Bracelets/1.png", "Jewelry/assets/Bracelets/2.png", "Jewelry/assets/Bracelets/3.png"]
alt_ear_images = ["Jewelry/assets/1.png", "Jewelry/assets/2.png", "Jewelry/assets/3.png", "Jewelry/assets/4.png"]
# Initialize WebRTC in the sidebar
with st.sidebar:
    st.header("Camera Feed")
    st.session_state['webrtc_ctx'] = camera_input_live(key="webrtc")
# Create tabs for different types of jewelry
tab1, tab2, tab3, tab4 = st.tabs(["Necklace", "Ring", "Earring", "Bracelet"])

def handle_camera_feed(selected_image, overlay_function, key):
    if 'webrtc_ctx' not in st.session_state:
        st.session_state['webrtc_ctx'] = camera_input_live(key=key)
    if st.session_state['webrtc_ctx'].video_processor:
        st.session_state['webrtc_ctx'].video_processor.update_overlay(overlay_function, selected_image)

with tab1:
    st.header("Necklace")
    selected_image_necklace = image_select("Select a Necklace", necklace_images, key="selected_image_necklace")
    if st.button("TRY ON", key="try_on_necklace"):
        handle_camera_feed(selected_image_necklace, overlay_jewelry, key="necklace")

with tab2:
    st.header("Ring")
    selected_image_ring = image_select("Select a Ring", ring_images, key="selected_image_ring")
    if st.button("TRY ON", key="try_on_ring"):
        handle_camera_feed(selected_image_ring, overlay_ring_on_hand, key="ring")
...
with tab3:
    ...
    index = ear_rings_images.index(selected_image_ear_ring)
    selected_image_ear_ring = alt_ear_images[index]
    if st.button("TRY ON", key="try_on_earring"):
        handle_camera_feed(selected_image_ear_ring, overlay_earring, key="earring")

with tab4:
    st.header("Bracelet")
    selected_image_bracelet = image_select("Select a Bracelet", bracelet_images, key="selected_image_bracelet")
    if st.button("TRY ON", key="try_on_bracelet"):
        handle_camera_feed(selected_image_bracelet, add_bracelet_overlay, key="bracelet")

# Always display the WebRTC video UI
# if 'webrtc_ctx' in st.session_state:
# st.session_state['webrtc_ctx'] = camera_input_live()
